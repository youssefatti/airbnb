...
router.get("/validation/:token", function(req, res) {
  ...
  Users.findOneAndUpdate(
    { token: token, activated: false },
    { activated: true, token: uid(32) },
    { new: true },
    function(err, user) {
      if (!err) {
        if (user != null) {
          // si le token existe et que le compte n'est pas activé alors on envoi le mail
          var data = {
      ...
      if (!err) {
        if (0 === user.length) {
          ...
          Users.findById({ _id: id }, function(err, user) {
            // alors on cherche la fiche demandé par le user du token
            var userResponse = {
              _id: user._id,
              account: {
                username: user.account.username,
...
router.post("/log_in", function(req, res) {
  ...
  if (itsEmail !== -1) {
    ...
    // Dans ce cas c'est un username
    username = input;
    Users.find({ "account.username": username }, function(err, user) {
      //Recherche user avec username
      if (err) {
        console.log("something went wrong");
      } else {
        var salt = user[0].salt; // Récupération du salt enregistré dans la fiche user
